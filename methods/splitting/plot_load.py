...
for output_name in output_list:
    ...
    data = pd.read_csv("./data/{}/load.csv".format(folder_name))
    disp = -data["step"].values
    load = data["load"].values
    plt.plot(disp,load,label="MPs - {}".format(output_name.split("-")[-1]),ls="--")



output_regex = re.compile("output-split-.*")
output_list = list(filter(output_regex.match,os.listdir(chalk_dir)))
# output_list.sort(key=lambda x: int(x.split("-")[-1]))
displacements = []
mps = []
for output_name in output_list:
    ...
    disp = -data["step"].values
    load = data["load"].values
    plt.plot(disp,load,label="MPs - {} split".format(output_name.split("-")[-1]))
