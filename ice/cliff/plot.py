...
for fname in flist:
    ...
    for i in [-1]:#range(len(data["frames"])):
        p = mpmplotter.plot.plot(data,i,colour_name="sig_xy",df_transform=transform)
        water_height = data["settings"]["OCEAN-HEIGHT"] - (h * offset)
        # patch = Rectangle(xy=(0,0), width=d_width, height=water_height,color="blue",fill=False,hatch="x",zorder=-5)
        patch = Rectangle(xy=(0,0),
                          ...
                          zorder=-5)
        # patch_sea = [patch]
        # ps = PatchCollection(patch_sea,zorder=-1)
        ax = plt.gca()
        ax.add_patch(patch)
